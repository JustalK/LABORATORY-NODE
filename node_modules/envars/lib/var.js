...
async function set(name, value, options) {
  ...
  const filename = _path.default.join(cwd, `.${envName}.env`);

  const overrideFilename = _path.default.join(cwd, `.${envName}.override.env`); // Ensure that that target directory exists.
  ...
  if (options.secret) {
    ...
    if (!password) {
      ...
      prl.close();
      const lines = await (0, _utils.readLines)(overrideFilename, encoding);
      let append = true;
      ...
      await (0, _utils.writeLines)(lines, overrideFilename, encoding);
    }
...
async function get(name, options) {
  ...
  const overrideFilename = _path.default.join(cwd, `.${envName}.override.env`);
  ...
  if (value && _cipher.encRegExp.test(value)) {
    const password = await (0, _utils.findPassword)(overrideFilename, encoding);
